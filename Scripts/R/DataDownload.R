...
### Calculate the frequency of each value in the 'site_name' column
value_counts <- table(SnotelData_AK$site_name)

### Identify sites that occur more than 250 times
frequent_values <- names(value_counts[value_counts > 250])

### Filter the dataframe to keep only rows where the site_name column value is in the 'frequent_values' vector
SnotelData_AK <- SnotelData_AK[SnotelData_AK$site_name %in% frequent_values, ]
...
### Calculate the frequency of each value in the 'site_name' column
value_counts <- table(SnotelData_CONUS$site_name)
...
### Filter the dataframe to keep only rows where the site_name column value is in the 'frequent_values' vector
SnotelData_CONUS <- SnotelData_CONUS |>
  filter(SnotelData_CONUS$site_name %in% frequent_values)
